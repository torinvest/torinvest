<?php
/**
 * API session USA War Atlas — login_formation_bridge / ping / logout.
 */
...
function atlasJson(array $data, int $status = 200): void
{
...
try {
    ...
    atlasJson(['ok' => false, 'error' => $e->getMessage()], 503);
}
...
try {
    switch ($action) {
        case 'login_formation_bridge':
            if ($method !== 'POST') {
                atlasJson(['ok' => false, 'error' => 'method_not_allowed'], 405);
            }
            atlasJson(atlasLoginFormationBridge((string) ($input['bridgeToken'] ?? '')));
        ...
        case 'ping':
            $session = atlasRequireSession();
            atlasJson(atlasPing($session));
        ...
        default:
            atlasJson(['ok' => false, 'error' => 'action_inconnue'], 400);
    }
} catch (AtlasAccessException $e) {
    atlasJson($e->payload, $e->getCode() > 0 ? $e->getCode() : 403);
} catch (InvalidArgumentException $e) {
    atlasJson(['ok' => false, 'error' => $e->getMessage()], 400);
    ...
    atlasJson(['ok' => false, 'error' => $msg], $status);
} catch (Throwable $e) {
    atlasJson(['ok' => false, 'error' => $e->getMessage()], 500);
}
