...
/**
 * Déblocage progressif — lots de 3 (intégré ici pour déploiement VPS sans forge-unlock.js).
 */
(function (global) {
  "use strict";
  ...
  function getMaxUnlockedModuleIndex() {
    ...
    while (batchStart + FORGE_UNLOCK_BATCH_SIZE <= ids.length) {
      ...
      maxIdx = Math.min(batchStart + FORGE_UNLOCK_BATCH_SIZE - 1, ids.length - 1);
    }
    return maxIdx;
  }
  ...
  function getUnlockSummaryText() {
    ...
    return (
      "Parcours par lots : " +
      getUnlockedModuleIds().length +
      " / " +
      ids.length +
      " modules ouverts · lot " +
      getCurrentUnlockedBatchNumber() +
      " / " +
      getUnlockBatchCount()
    );
  }
